...
本仓库提供面向 **百度地图 RTOS SDK（mapsdk-rtos）** 的 AI Coding Agent Skill `baidu-map-rtos-skills`，帮助开发者在智能研发工具（如 Cursor / Claude Code 等）中高效完成 RTOS 地图应用层集成、Canvas 适配、地图组件控制、覆盖物绘制、检索路线、离线地图与导航等开发任务。
...
适用于对接 `includes/` 下 `*_api.h` 公开头文件的 RTOS 地图应用开发场景。
...
- **开发语言**：C / C++
- **SDK**：mapsdk-rtos
- **核心 API**：`MapViewApi`、`AuthLicenseApi`、`SearchApi`、`NaviApi`、`MapOfflineApi`
- **集成方式**：对接 `includes/` 公开头文件与平台 Adapter / Canvas 实现
...
在支持 Skills 的客户端里，当你的问题涉及「RTOS 地图 SDK」「mapsdk-rtos」「MapViewApi」「AuthLicenseApi」「SearchApi」「NaviApi」「MapOfflineApi」「Canvas 适配」「RTOS 覆盖物」「RTOS 导航」「RTOS 离线地图」「多实例地图」等关键词时，助手会优先参考本仓库文档来回答，从而给出更贴合百度地图 RTOS SDK 的代码与用法。
